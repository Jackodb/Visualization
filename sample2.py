...
def update_figure(n_clicks, input1,input2):
    ...
    data=[
        go.Bar(
            x=list(current_dict.values()), # length of the dict (len(dict))
            y=[i for i in range(len(list(df.Price)))], # all values from current_dict
            name='Buy',
        ...
        go.Bar(
            x=list(initial_dict.values()), # length of the dict
            y=[i for i in range(len(list(df.Price)))], # all values from mountain_dict
            name='Sell',
            marker=go.bar.Marker(
